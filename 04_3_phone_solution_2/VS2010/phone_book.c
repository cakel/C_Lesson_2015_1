...
void addNewData(void)
{
    ...
    Person *curNode = NULL;                            // 리스트상의 현재 노드
    Person *preNode = NULL;                            // 리스트상의 바로 직전 노드
    Person *newNode	= NULL;                            // 새로운 연락처 노드
    char *upperNewStr	= NULL;                          // 대문자로 변경시킨 새로운 연락처 이름
    char *upperNodeStr	= NULL;                         // 대문자로 변경시킨 기존의 연락처 이름
	char name[30] = {0};
	char phone[30] = {0};
...
void deleteData(void)
{
	...
	char str[20] = {0};                           // 이름 or 전화번호 저장
    char yn = 0;                                // 삭제 여부 저장
    Person *tNode = NULL;                          // 리스트상의 현재 위치 저장
	...
	if(tNode != NULL)
	{
		...
		if(yn == 'y')
		{
			...
			{
				// 중간인 경우 이전 Node 를 알아 낸 후 삭제할 다음 Node 와 연결 후 삭제
				while(sNode != NULL)
				{
...
Person *inputData(void)
{
	char name[30] = {0};                              // 이름 저장
	char phone[30] = {0};                             // 전화번호 저장
